...
class Board {
    ...
    public:
        ...
        static constexpr int MAX_TURNS = 60;
        ...
        ~Board() { monopolyUtil::closeLogFile(); }; // Closes the log file to avoid leaving open resources at the end of the program
        const std::vector<std::shared_ptr<Player>>& getPlayers(void) const { return this->players; };
        void print(void); // Prints the board and the current game stats (tiles owned by each player and their current balance)
        ...
        friend void monopolyUtil::gameLoop(Board board); // Declares the gameLoop function as a friend of the Board class, so it can access the private field "players" of the Board class
};
